...
2b. **H1 (deferred from Phase 01) — true darkness:** ✅ **Shipped `be9faae`.** Added `isComingSoon()` to `lib/environment.ts`; `robots.ts` + `sitemap.ts` block on `!isProductionDeployment() || isComingSoon()`; the 4 `llms.txt`/`llms-full.txt` routes return a minimal stub on `isProductionDeployment() && isComingSoon()` (local/staging still serve full for dev). Type-clean. Prod verify: robots `Disallow: /` ✅, empty sitemap ✅. Follow-up `d2f53e5`: llms routes are `revalidate`-static so `isProductionDeployment()` was false at build (served catalog on dark prod) — re-gated llms on `isComingSoon()` alone (build-safe). **Verified on prod 2026-06-05:** robots `Disallow: /`, sitemap 0 urls, root `/llms.txt` + `/llms-full.txt` stub, locale `llms` redirect to holding page. ✅ **H1 DONE.**
