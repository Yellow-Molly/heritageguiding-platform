...
- Depends on: [phase-03-result-fetcher-parser.md](./phase-03-result-fetcher-parser.md), [phase-04-html-report-renderer.md](./phase-04-html-report-renderer.md)
- Thresholds defined in: [phase-01-scaffolding-config-env.md](./phase-01-scaffolding-config-env.md) (`DIFF_THRESHOLDS`)
- Carries over from predecessor: `plans/260516-2135-multi-location-lighthouse-report/phase-05-diff-vs-previous-run.md` (no functional changes; data shape identical)
...
## Key Insights
- Previous run = immediately preceding sibling directory by name sort (names `multi-location-{YYMMDD-HHmm}` → lexicographic == chronological)
- Skip diff cleanly when no previous run exists; never error
- Direction convention: scores ↑ is good (green), CWV metrics ↓ is good (green except CLS which lower = better)
- Threshold-exceeded deltas: bold + arrow icon; sub-threshold deltas: small muted text
...
- CLI flag `--no-diff` to skip overlay even when previous exists
- First-run auto-skip: no previous → log "first run, no diff" and proceed

### Non-functional
- Pure functions, deterministic
- New file under ~100 LOC
...
5. Mismatched runs (e.g. previous lacked `tour-details` mobile) render current only, no broken layout
...
| Wrong run picked as "previous" (e.g. aborted/empty run) | Filter previous summaries with `successful === 0` → skip |
