...
- Depends on: [phase-03-result-parser.md](./phase-03-result-parser.md)
...
- Tabbed mobile/desktop view: lightweight `<details>`/`<summary>` or radio-input hack — zero JS option preferred for KISS
- Accessibility findings are deterministic by page — merge mobile + desktop a11y audits, dedupe by `id`, render once per page
...
- Header section: timestamp, git SHA (short), staging URL, total submissions / success / failed, run duration, link to previous run if present
...
- Each page section contains:
  ...
  - **CWV table**: rows = location, columns = TTFB / FCP / LCP / TBT / CLS / SI, one block per form factor (or combined with form-factor column)
  - **Accessibility findings**: deduped list of failed audits for that page across all locations + form factors, with severity badges and links to WPT reports for evidence
- **TourDetails caveat block**: callout box explaining Bokun widget dominates LCP and the score reflects third-party load, not the host page
...
- Print stylesheet not required, but readable B&W if printed
- No JS dependencies; ≤30 LOC inline JS for tab toggling (acceptable; or use `<details>`)
...
3. Tab UX choice: use HTML `<details open>` for each form-factor block — zero JS, works everywhere. Reject radio-input hack; it's overkill.
...
- [ ] Create `report-template.ts` with shell + atom helpers (scoreBadge, metricCell, severityBadge, htmlShell)
...
4. CWV table: numeric values aligned right, units displayed, "—" for nulls
...
7. Header shows timestamp, git SHA, staging URL, success/failed counts
...
| Inline CSS conflicts with browser defaults | CSS reset block at top of style sheet |

## Security Considerations
- All dynamic content HTML-escaped before insertion (write tiny `esc()` helper)
